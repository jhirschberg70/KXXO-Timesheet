...
function editCheck(event) {
  ...
  if (target == 'edit-holiday') {
	...
	$(targetSelector).siblings().children('.toggle-status').html('Talent');
	$(targetSelector).closest('.col-auto').siblings(':last').find('input').prop('disabled', false);
	$(targetSelector).closest('.col-auto').siblings(':last').addClass('invalid');
      }
      else {
	$(targetSelector).siblings().children('.toggle-status').html('Regular');
	$(targetSelector).closest('.col-auto').siblings(':last').find('input').prop('disabled', true);
	$(targetSelector).closest('.col-auto').siblings(':last').removeClass('invalid');
      }
    }
  ...
  function allTimesValid() {
	...
	allTimesValid = false;
      ...
      // If hoursType is Talent and no valid rate is filled in, times are invalid
      if (hoursType === 'Talent') {
      }    
    }
